...
Thumbnailer.prototype.execute = function (description, localPaths, onComplete) {
  ...
  _.extend(this, {
    ...
    background: (description.background || 'white'),
...
Thumbnailer.prototype.matted = function () {
  ...
  var thumbnailCommand = config.get('convertCommand') + ' "' + this.localPaths[0] + '" ' + autoOrient + ' -resize ' + dimensionsString + ' -size ' + dimensionsString + ' xc:' + this.background + ' +swap -gravity center' + qualityString + ' -composite ' + this.convertedPath
...
Thumbnailer.prototype.bounded = function () {
  ...
  var qualityString = (this.quality ? '-quality ' + this.quality + ' ' : '')
  var autoOrient = (this.autoOrient ? '-auto-orient' : '')
  var thumbnailCommand = config.get('convertCommand') + ' "' + this.localPaths[0] + '" ' + autoOrient + ' -thumbnail ' + dimensionsString + ' -alpha remove ' + qualityString + this.convertedPath
...
Thumbnailer.prototype.fill = function () {
  ...
  var qualityString = (this.quality ? '-quality ' + this.quality : '')
  var autoOrient = (this.autoOrient ? '-auto-orient' : '')
  var thumbnailCommand = config.get('convertCommand') + ' "' + this.localPaths[0] + '" ' + autoOrient + ' -resize ' + dimensionsString + '^ -gravity center -extent ' + dimensionsString + ' ' + qualityString + ' ' + this.convertedPath
...
Thumbnailer.prototype.strict = function () {
  ...
  var qualityString = (this.quality ? '-quality ' + this.quality : '')
  var autoOrient = (this.autoOrient ? '-auto-orient' : '')
  var thumbnailCommand = config.get('convertCommand') + ' "' + this.localPaths[0] + '" ' + autoOrient + ' -resize ' + dimensionsString + '! ' + qualityString + ' ' + this.convertedPath
