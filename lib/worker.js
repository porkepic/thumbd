...
var { SQSClient, GetQueueUrlCommand, ReceiveMessageCommand, DeleteMessageCommand } = require('@aws-sdk/client-sqs')
...
function Worker (opts) {
  ...
  this.sqs = new SQSClient({ region: config.get('awsRegion') })

  this.getQueueUrl().then((queueUrl) => {
    config.set('sqsQueueUrl', queueUrl)
  })
}
...
Worker.prototype.getQueueUrl = function () {
  return new Promise((resolve, reject) => {
    if (config.get('sqsQueueUrl')) {
      resolve(config.get('sqsQueueUrl'))
    }
    this.sqs.send(new GetQueueUrlCommand({ QueueName: config.get('sqsQueue') })).then((data) => {
      config.set('sqsQueueUrl', data.QueueUrl)
...
Worker.prototype._processSQSMessage = function () {
  ...
  this.getQueueUrl().then((queueUrl) => {
    this.sqs.send(new ReceiveMessageCommand({ QueueUrl: queueUrl, MaxNumberOfMessages: 1, WaitTimeSeconds: 60 })).then((job) => {
...
Worker.prototype._deleteJob = function (handle) {
  ...
  this.getQueueUrl().then((queueUrl) => {
    _this.sqs.send(new DeleteMessageCommand({ QueueUrl: queueUrl, ReceiptHandle: handle })).then((resp) => {
...
Worker.prototype._notify = function(job, cb) {
  ...
  if (!job.notify) return cb();
	...
	request.post(options, function(err) {
		...
		if (!err){
			_this.logger.error('notified:', job.notify);
		}else{
			_this.logger.error("notified with error:", err);
		}
		return cb();
	});
};
