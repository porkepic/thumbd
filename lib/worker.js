...
function Worker (opts) {
  ...
  config.set('sqsQueueUrl', config.get('sqsQueue'))
}
...
Worker.prototype.start = function () {
  this.logger.info('wait for message on ' + config.get('sqsQueueUrl'))
...
Worker.prototype._processSQSMessage = function () {
  ...
  this.sqs.send(new ReceiveMessageCommand({ QueueUrl: config.get('sqsQueueUrl'), MaxNumberOfMessages: 1, WaitTimeSeconds: 20 })).then((job) => {
...
Worker.prototype._deleteJob = function (handle) {
  ...
  _this.sqs.send(new DeleteMessageCommand({ QueueUrl: config.get('sqsQueueUrl'), ReceiptHandle: handle })).then((resp) => {
